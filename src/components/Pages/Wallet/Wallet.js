...
export const Wallet = ({address, setAddress, nfts, setNfts, setTask, balances, setBalances}) => {
    ...
    const [activeWarning, setActiveWarning] = useState(true); const [activeWrapBtn, setActiveWrapBtn] = useState(false);
    ...
    useEffect(() => {
        if (selectedToken === null) {
            if (selectedNFTs != "{}") {setActiveSendBtn(true)} else { setActiveSendBtn(false)}
        } else {
            setActiveSendBtn(true)
        }
    }, [selectedToken])
    ...
    return (
        <div className={classes.root} style={{backgroundImage: `url(${bg})`}} onClick={() =>
        { if (clickedMenu) { setClickedMenu(false); } }}>
            ...
            {address ? <>
                ...
                <SendForm active={activeSendForm} nfts={nfts} balances={balances} setBalances={setBalances}
                          setActive={setActiveSendForm} selected={selectedNFTs != "{}" ? selectedNFTs : selectedWNFTs}
                          selToken={selectedToken} setNFTs={setNfts}/>
                <MenuBar address={address} clicked={clickedMenu} setClicked={setClickedMenu} curLink='wallet' setTask={setTask}>
                    <div className={classes.buttons}>
                        <Button active={activeWrapBtn} style={{}} buttonType="middleBtn" onClick={() => {
                            if (selectedWNFTs === "{}") {
                                setWait(true)
                        ...
                        }}>{selectedWNFTs === "{}" ? "Wrap" : "Unwrap"}</Button>
                        <Button active={activeSendBtn} style={{}} buttonType="middleBtn" selected={setSelectedNFTs} onClick={() => {
                ...
                </MenuBar> {/*<Resources address={address}/>*/}
                <Filter filt={filt} setFilt={setFilt}/>
                {JSON.parse(filt).tokens ?
                    <Tokens address={address} selected={selectedToken} setSelected={setSelectedToken}
                            selectedNFTs={selectedNFTs} setSelectedNFTs={setSelectedNFTs}
                            balances={balances} setBalances={setBalances}
                            selectedWNFTs={selectedWNFTs} setSelectedWNFTs={setSelectedWNFTs}
                    />
                    : null
                }
                <NFTs address={address} nfts={nfts}
                      filt={filt} selected={selectedNFTs} setSelected={setSelectedNFTs} setSelectedToken={setSelectedToken}
                      selectedWNFTs={selectedWNFTs} setSelectedWNFTs={setSelectedWNFTs}/>
            </> : null}
        </div>
    )
